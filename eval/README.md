...
可选参数：`--ping` 只发 1 次请求并打印平台的原始返回，用于排查连接、key 和模型名；`--limit <N>` 只评估前 N 条草稿和前 N 条消息，适合先小规模试跑；`--model <模型名>` 更换模型；`--rpm <数字>` 调整每分钟请求数，遇到限速时调低；`--out <名称>` 指定结果文件名。结果写入 results/，默认文件名为 `llm-<provider>.json`。运行时每完成一次请求会打印一行进度；遇到 key 无效、模型不存在或当日额度用完，脚本会立即停止并说明原因。
